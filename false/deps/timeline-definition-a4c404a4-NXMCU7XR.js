...
// node_modules/.pnpm/registry.npmmirror.com+mermaid@10.2.0/node_modules/mermaid/dist/timeline-definition-a4c404a4.js
var Y = function() {
  var n = function(k, r, a, h) {
    for (a = a || {}, h = k.length; h--; a[k[h]] = r)
      ;
    return a;
  }, t = [1, 2], e = [1, 5], s = [6, 9, 11, 17, 18, 20, 22, 23, 26, 27, 28], i = [1, 15], o = [1, 16], l = [1, 17], y = [1, 18], f = [1, 19], d = [1, 23], m = [1, 24], S = [1, 27], _ = [4, 6, 9, 11, 17, 18, 20, 22, 23, 26, 27, 28], b2 = {
    trace: function() {
    },
    yy: {},
    ...
    parse: function(r) {
      ...
      for (var w2, V, N, Q, F = {}, q, $, nt, U; ; ) {
        if (V = h[h.length - 1], this.defaultActions[V] ? N = this.defaultActions[V] : ((w2 === null || typeof w2 > "u") && (w2 = et()), N = M[V] && M[V][w2]), typeof N > "u" || !N.length || !N[0]) {
          var X = "";
          U = [];
          for (q in M[V])
...
Expecting ` + U.join(", ") + ", got '" + (this.terminals_[w2] || w2) + "'" : X = "Parse error on line " + (C + 1) + ": Unexpected " + (w2 == j ? "end of input" : "'" + (this.terminals_[w2] || w2) + "'"), this.parseError(X, {
        ...
        if (N[0] instanceof Array && N.length > 1)
          throw new Error("Parse Error: multiple actions possible at state: " + V + ", token: " + w2);
        switch (N[0]) {
          case 1:
            h.push(w2), g.push(p.yytext), c.push(p.yylloc), h.push(N[1]), w2 = null, O = p.yyleng, x = p.yytext, C = p.yylineno, H = p.yylloc;
            break;
          case 2:
...
var ht = function() {
  D.length = 0, J.length = 0, W = "", B.length = 0, Qa();
};
var dt = function(n) {
  W = n, D.push(n);
};
...
var xt = function() {
  ...
  function t(i, o, l, y, f, d, m, S, _) {
    const { taskFontSize: b2, taskFontFamily: v2 } = S, I = i.split(/<br\s*\/?>/gi);
    for (let k = 0; k < I.length; k++) {
      const r = k * b2 - b2 * (I.length - 1) / 2, a = o.append("text").attr("x", l + f / 2).attr("y", y).attr("fill", _).style("text-anchor", "middle").style("font-size", b2).style("font-family", v2);
  ...
  function e(i, o, l, y, f, d, m, S) {
    const _ = o.append("switch"), v2 = _.append("foreignObject").attr("x", l).attr("y", y).attr("width", f).attr("height", d).attr("position", "fixed").append("xhtml:div").style("display", "table").style("height", "100%").style("width", "100%");
    v2.append("div").attr("class", "label").style("display", "table-cell").style("text-align", "center").style("vertical-align", "middle").text(i), t(i, _, l, y, f, d, m, S), s(v2, m);
  }
  function s(i, o) {
    for (const l in o)
      l in o && i.attr(l, o[l]);
...
function _t(n, t) {
  n.each(function() {
    var e = Ct(this), s = e.text().split(/(\s+|<br>)/).reverse(), i, o = [], l = 1.1, y = e.attr("y"), f = parseFloat(e.attr("dy")), d = e.text(null).append("tspan").attr("x", 0).attr("y", y).attr("dy", f + "em");
...
`), b.debug("timeline", s.db);
  ...
  _.forEach(function(P) {
    ...
    b.debug("sectionHeight before draw", E), b2 = Math.max(b2, E + 20);
  });
  let g = 0, c = 0;
  b.debug("tasks.length", m.length);
  for (const [P, p] of m.entries()) {
    ...
    b.debug("taskHeight before draw", L), v2 = Math.max(v2, L + 20), g = Math.max(g, p.events.length);
    let H = 0;
  ...
  b.debug("maxSectionHeight before draw", b2), b.debug("maxTaskHeight before draw", v2), _ && _.length > 0 ? _.forEach((P) => {
    ...
    b.debug("sectionNode", E);
    const L = d.append("g"), H = R.drawNode(L, E, h, i);
    b.debug("sectionNode output", H), L.attr("transform", `translate(${r}, ${k})`), a += b2 + 50, p.length > 0 && at(
      d,
      p,
      h,
      r,
  ...
  b.debug("bounds", M), S && d.append("text").text(S).attr("x", M.width / 2 - o).attr("font-size", "4ex").attr("font-weight", "bold").attr("y", 20), I = u ? b2 + v2 + 150 : v2 + 100, d.append("g").attr("class", "lineWrapper").append("line").attr("x1", o).attr("y1", I).attr("x2", M.width + 3 * o).attr("y2", I).attr("stroke-width", 4).attr("stroke", "black").attr("marker-end", "url(#arrowhead)"), Bm(
    void 0,
    d,
...
var at = function(n, t, e, s, i, o, l, y, f, d, m) {
  ...
  for (const _ of t) {
    ...
    b.debug("taskNode", b2);
    const v2 = n.append("g").attr("class", "taskWrapper"), k = R.drawNode(v2, b2, e, l).height;
    if (b.debug("taskHeight after draw", k), v2.attr("transform", `translate(${s}, ${i})`), o = Math.max(o, k), _.events) {
...
var Ut = (n) => {
  ...
  for (let e = 0; e < n.THEME_COLOR_LIMIT; e++)
    n["lineColor" + e] = n["lineColor" + e] || n["cScaleInv" + e], ri(n["lineColor" + e]) ? n["lineColor" + e] = v(n["lineColor" + e], 20) : n["lineColor" + e] = w(n["lineColor" + e], 20);
  for (let e = 0; e < n.THEME_COLOR_LIMIT; e++) {
    const s = "" + (17 - 3 * e);
    t += `
...
//# sourceMappingURL=timeline-definition-a4c404a4-NXMCU7XR.js.map
