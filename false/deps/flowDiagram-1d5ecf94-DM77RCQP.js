import {
  ...
  we,
  ye
} from "./chunk-CAMGUD44.js";
import "./chunk-I4RTZAPP.js";
import {
  Je,
  et
...
// node_modules/.pnpm/registry.npmmirror.com+mermaid@10.2.0/node_modules/mermaid/dist/flowDiagram-1d5ecf94.js
function Dt(r2) {
  if (!r2.ok)
...
function de() {
  ...
  return r2.createNodes = function(e) {
    ...
    return arguments.length ? (Ut(e), r2) : Y;
  }, r2;
}
var he2 = {
  paddingLeft: 10,
...
var bt = function(r2, e, t, n, a, s) {
  ...
  Object.keys(r2).forEach(function(d) {
    ...
    if (Ba(Et().flowchart.htmlLabels)) {
      ...
      const k2 = u.split(zn.lineBreakRegex);
      for (const B of k2) {
    ...
    b2.warn("Adding node", l.id, l.domId), e.setNode(s.db.lookUpDomId(l.id), {
      labelType: "svg",
...
var kt = function(r2, e, t) {
  ...
  r2.forEach(function(i) {
    ...
    l.style = y, l.labelStyle = h, i.interpolate !== void 0 ? l.curve = Z0(i.interpolate, Kf) : r2.defaultInterpolate !== void 0 ? l.curve = Z0(r2.defaultInterpolate, Kf) : l.curve = Z0(xt.curve, Kf), i.text === void 0 ? i.style !== void 0 && (l.arrowheadStyle = "fill: #333") : (l.arrowheadStyle = "fill: #333", l.labelpos = "c", Ba(Et().flowchart.htmlLabels) ? (l.labelType = "html", l.label = `<span id="L-${o}" class="edgeLabel L-${c2}' L-${d}" style="${l.labelStyle}">${i.text.replace(
      /fa[blrs]?:fa-[\w-]+/g,
      (u) => `<i class='${u.replace(":", " ")}'></i>`
    )}</span>`) : (l.labelType = "text", l.label = i.text.replace(zn.lineBreakRegex, `
...
var we2 = function(r2, e) {
  b2.info("Extracting classes"), e.db.clear();
  try {
...
var xe = function(r2, e, t, n) {
  b2.info("Drawing flowchart"), n.db.clear();
  const { securityLevel: a, flowchart: s } = Et();
  let i;
  a === "sandbox" && (i = Ct("#i" + e));
  ...
  const k2 = new de();
  ge.addToRender(k2), k2.arrows().none = function(b3, L, w, M) {
    const C = b3.append("marker").attr("id", L).attr("viewBox", "0 0 10 10").attr("refX", 9).attr("refY", 5).attr("markerUnits", "strokeWidth").attr("markerWidth", 8).attr("markerHeight", 6).attr("orient", "auto").append("path").attr("d", "M 0 0 L 0 0 L 0 0 z");
    O(C, w[M + "Style"]);
  }, k2.arrows().normal = function(b3, L) {
    b3.append("marker").attr("id", L).attr("viewBox", "0 0 10 10").attr("refX", 9).attr("refY", 5).attr("markerUnits", "strokeWidth").attr("markerWidth", 8).attr("markerHeight", 6).attr("orient", "auto").append("path").attr("d", "M 0 0 L 10 5 L 0 10 z").attr("class", "arrowheadPath").style("stroke-width", 1).style("stroke-dasharray", "1,0");
  };
  ...
  for (k2(E, h), E.selectAll("g.node").attr("title", function() {
    ...
    if (u = p[v], u.title !== "undefined") {
      ...
      ), b3 = c2.querySelectorAll(
        "#" + e + ' [id="' + n.db.lookUpDomId(u.id) + '"]'
      ), L = g[0].x.baseVal.value, w = g[0].y.baseVal.value, M = g[0].width.baseVal.value, C = Ct(b3[0]).select(".label");
      C.attr("transform", `translate(${L + M / 2}, ${w + 14})`), C.attr("id", e + "Text");
      for (let W = 0; W < u.classes.length; W++)
        b3[0].classList.add(u.classes[W]);
    }
  if (!s.htmlLabels) {
    const g = c2.querySelectorAll('[id="' + e + '"] .edgeLabel .label');
    for (const b3 of g) {
      const L = b3.getBBox(), w = c2.createElementNS("http://www.w3.org/2000/svg", "rect");
      w.setAttribute("rx", 0), w.setAttribute("ry", 0), w.setAttribute("width", L.width), w.setAttribute("height", L.height), b3.insertBefore(w, b3.firstChild);
    }
  }
  Bm(h, B, s.diagramPadding, s.useMaxWidth), Object.keys(m).forEach(function(g) {
    const b3 = m[g];
    if (b3.link) {
      const L = o.select("#" + e + ' [id="' + n.db.lookUpDomId(g) + '"]');
      if (L) {
        const w = c2.createElementNS("http://www.w3.org/2000/svg", "a");
        w.setAttributeNS("http://www.w3.org/2000/svg", "class", b3.classes.join(" ")), w.setAttributeNS("http://www.w3.org/2000/svg", "href", b3.link), w.setAttributeNS("http://www.w3.org/2000/svg", "rel", "noopener"), a === "sandbox" ? w.setAttributeNS("http://www.w3.org/2000/svg", "target", "_top") : b3.linkTarget && w.setAttributeNS("http://www.w3.org/2000/svg", "target", b3.linkTarget);
...
//# sourceMappingURL=flowDiagram-1d5ecf94-DM77RCQP.js.map
